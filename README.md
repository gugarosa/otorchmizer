...
Did you ever reach a bottleneck in your computational experiments? Are you tired of waiting hours for optimization runs?
Otorchmizer provides **97 optimization and search algorithms**, powered by PyTorch tensors for CPU and CUDA execution.
From populations to search spaces, from internal functions to external communication, we support optimization research.
...
**Version 2.0 restores the canonical migration targets.** It adds the six missing
optimizers and repairs the documented identity, equation, phase, and parameter gaps.
Read [Migration contracts and limits](docs/algorithm_limits.rst) for compatibility
changes, deliberate corrections to reference defects, and device limitations.
...
The optimizer families expose **97 optimizer classes**. Implementations use tensor
...
For GPU acceleration, select a PyTorch build matching the available driver.
With uv, this also replaces an already-installed CPU-only build:
...
With pip, use the command generated by the [PyTorch installation selector](https://pytorch.org/get-started/locally/).
A GPU can be present while `torch.cuda.is_available()` is false when the installed wheel is CPU-only.
...
| **Algorithms** | 97 optimizer classes in 3.1.4 | 97 exported optimizer classes |
...
The export inventory matches Opytimizer 3.1.4, including genetic programming and
nondominated sorting. Matching inventories do not imply identical random streams,
trajectories, or universal backend support. See [Migration contracts and limits](docs/algorithm_limits.rst).
...
| [Migration contracts and limits](docs/algorithm_limits.rst) | Canonical algorithm identities, compatibility changes, and device limits |
